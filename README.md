...
if [[ "\$2" == "up" && \("\$CURRENT_SSID" == "BITS-STUDENT" || "\$CURRENT_SSID" == "BITS-STAFF"\) ]];
    then sleep 3 su -c "/home/$(whoami)/path/to/bits-wifi-login/fortinet-login.sh >> /tmp/fortinet-login.log 2>&1 &" $(whoami)
fi
EOF
```
